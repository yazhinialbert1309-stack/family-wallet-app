...
export default function Home() {
  ...
  const BUDGET_LIMIT = 10000; // Budget Warning Limit
    ...
    <main className="min-h-screen bg-[#F8F9FE] p-4 md:p-8 text-black font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 overflow-x-auto pb-2">
          {['Jan', 'Feb', 'Mar'].map((m) => (
            <button key={m} onClick={() => setActiveMonth(m)} className={`px-8 py-3 rounded-2xl font-bold flex items-center gap-2 transition-all ${activeMonth === m ? 'bg-purple-600 text-white shadow-lg' : 'bg-white text-gray-400 border border-gray-100'}`}>
        ...
        {isOverBudget && (
          <div className="bg-rose-50 border-l-8 border-rose-500 p-6 rounded-3xl mb-8 flex items-center gap-4 shadow-sm">
            ...
            <div>
              ...
              <p className="text-rose-600 font-medium text-sm">இந்த மாத பட்ஜெட் வரம்பைத் தாண்டிவிட்டீர்கள்!</p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white p-8 rounded-[35px] shadow-sm border border-gray-100 h-fit">
            ...
            <div className="space-y-4 font-medium">
              ...
              <select className="w-full p-4 bg-gray-50 rounded-2xl outline-none" value={reason} onChange={(e) => setReason(e.target.value)}>
                <option>Food & Grocery 🍎</option><option>Rent 🏠</option><option>Electricity ⚡</option><option>Mobile Recharge 📱</option>
              </select>
            ...
            <div className="w-full h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={[{ month: activeMonth, expense: currentMonthStats.expense, savings: currentMonthStats.savings }]}>
                  ...
                  <Bar dataKey="savings" name="Savings" stackId="a" fill="#10B981" barSize={80} radius={[15, 15, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 mb-10 overflow-x-auto">
          ...
          {currentMonthStats.transactions.length === 0 && <p className="text-center py-20 text-gray-300 font-bold italic">No Activities</p>}
        </div>
      </div>
    </main>
  );
